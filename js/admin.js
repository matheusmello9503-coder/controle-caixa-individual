...
document.querySelectorAll('.aba-item').forEach(aba => {
    aba.addEventListener('click', () => {
        document.querySelectorAll('.aba-item').forEach(a => a.classList.remove('ativa'));
...
function renderizarResumo() {
    ...
    grade.innerHTML = `
        ...
        ${cartaoResumo('Total Geral', totalGeral, listaDoDia.length)}
    `;
...
function cartaoResumo(rotulo, valor, quantidade) {
    return `
...
async function carregarUsuarios() {
    ...
    document.getElementById('corpoUsuarios').innerHTML = usuarios.map(u => `
        <tr>
            <td>${u.nome}</td>
            <td>${u.email || '-'}</td>
            <td>${u.perfil === 'admin' ? 'Administrador' : 'Recepcao'}</td>
